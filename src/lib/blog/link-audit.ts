...
import { articleSlugExists, getAllArticleContents } from '@/lib/blog/supabase-blog';
...
async function replaceBrokenLinksSnapshot(slug: string, links: BrokenLink[]): Promise<void> {
  ...
  const { error: deleteError } = await supabase
    .from('blog_broken_links')
    .delete()
    .eq('article_slug', slug);
  if (deleteError) console.warn('[link-audit] delete:', deleteError.message);

  if (links.length > 0) {
    const { error } = await supabase.from('blog_broken_links').insert(
